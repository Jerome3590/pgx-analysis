...
For each `(cohort, age_band, item_type)` combination, the following files should be generated:

#### Data Files (`outputs/{cohort}/{age_band}/`)

| File Pattern | Description | Required |
|--------------|-------------|----------|
| `trajectory_results_{item_type}.json` | Complete DTW analysis results | ✅ Yes |
| `patient_trajectories_{item_type}.parquet` | Patient trajectory sequences | ✅ Yes |
| `cluster_assignments_{item_type}.csv` | Patient-to-cluster mappings | ✅ Yes |
| `archetype_trajectories_{item_type}.json` | Cluster archetype trajectories | ✅ Yes |

**Item Types:** `drug`, `icd`, `cpt`

**Example Files:**
- `outputs/opioid_ed/0_12/trajectory_results_drug.json`
- `outputs/opioid_ed/0_12/patient_trajectories_drug.parquet`
- `outputs/opioid_ed/0_12/cluster_assignments_drug.csv`
- `outputs/opioid_ed/0_12/archetype_trajectories_drug.json`

#### Visualization Files (`outputs/plots/`)

| File Pattern | Description | Required |
|--------------|-------------|----------|
| `{cohort}_{age_band}_{item_type}_similarity_heatmap.png` | DTW similarity matrix heatmap | ✅ Yes |
| `{cohort}_{age_band}_{item_type}_cluster_distribution.png` | Cluster size distribution | ✅ Yes |
| `{cohort}_{age_band}_{item_type}_sequence_length.png` | Average sequence length by cluster | ✅ Yes |
| `{cohort}_{age_band}_{item_type}_top_items.png` | Most common items across clusters | ✅ Yes |
| `{cohort}_{age_band}_{item_type}_archetype_visualization.png` | Archetype trajectory visualization | ⚠️ Optional |

**Example Files:**
- `outputs/plots/opioid_ed_0_12_drug_similarity_heatmap.png`
- `outputs/plots/opioid_ed_0_12_drug_cluster_distribution.png`
- `outputs/plots/opioid_ed_0_12_drug_sequence_length.png`
- `outputs/plots/opioid_ed_0_12_drug_top_items.png`

### Completion Checklist

For each cohort/age-band/item-type combination:

- [ ] Trajectory results JSON exists
- [ ] Patient trajectories Parquet file exists
- [ ] Cluster assignments CSV exists
- [ ] Archetype trajectories JSON exists
...
|--------------|-------------|----------|------------|
| `dtw_features_{cohort}_{age_band}.csv` | DTW trajectory features (distances to prototypes) | ✅ Yes | `create_dtw_features.py` |
| `dtw_added_features_{cohort}_{age_band}.csv` | **Final merged DTW features ready for model training** | ✅ Yes | `add_dtw_features_to_model_data.py` |

**S3 Locations:**
- DTW features: `s3://pgxdatalake/gold/feature_engineering/6_dtw/{cohort}/{age_band}/dtw_features_{cohort}_{age_band}.csv`
- Final merged features: `s3://pgxdatalake/gold/feature_engineering/6_dtw/{cohort}/{age_band}/dtw_added_features_{cohort}_{age_band}.csv`

**Format:** CSV with `mi_person_key` column for joining with `model_data` in final model step.

**Workflow:**
1. DTW scripts (`dtw_cohort_analysis.py` / `dtw_trajectory_analysis.py`) generate trajectory clusters → saves JSON/Parquet files
2. `create_dtw_features.py` creates patient-level features from trajectories → saves `dtw_features_{cohort}_{age_band}.csv`
3. `add_dtw_features_to_model_data.py` creates final merged file → saves `dtw_added_features_{cohort}_{age_band}.csv`

**Feature Engineering Checklist:**
- [ ] Run `create_dtw_features.py` to generate patient-level features from trajectories
- [ ] Verify `dtw_features_{cohort}_{age_band}.csv` exists in `outputs/feature_engineering/`
- [ ] Run `add_dtw_features_to_model_data.py` to create final merged feature file
- [ ] Verify `dtw_added_features_{cohort}_{age_band}.csv` exists in `outputs/feature_engineering/`
...
## 🎯 Primary Role: Feature Filtering (Noise Reduction)
...
- **Method**: Use DTW time windows to detect events occurring too close together (< 7 days)
- **Benefit**: Reduces noise, improves signal quality for downstream analyses
- **Output**: `model_events_no_protocols.parquet` (filtered data)

**See**: `PROTOCOL_FILTERING.md` for details

## 📊 Secondary Role: Feature Engineering
...
**Note**: Results may show protocol patterns rather than predictive signals
...
**Note**: DTW features showed non-significant differences between targets/controls, suggesting protocol patterns
