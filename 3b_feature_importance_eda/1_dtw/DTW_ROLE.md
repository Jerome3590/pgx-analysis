...
- **Recommendation**: Include but don't prioritize; focus on filtering first
...
python 6_dtw_analysis/create_dtw_features.py \
    --cohort opioid_ed \
    --age_band 0-12
```
...
1. ⚠️ **DTW features**: Include but don't prioritize
...
- Don't prioritize over filtering

## Next Steps

1. **Use filtered data**: Run all feature engineering with `model_events_no_protocols.parquet`
2. **Compare performance**: Test model with/without protocol filtering
3. **Refine threshold**: Adjust `min-interval-days` based on results
4. **Keep DTW features**: Include but don't prioritize in feature selection
