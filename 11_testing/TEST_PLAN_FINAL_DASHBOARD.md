...
Tests are **organized by dashboard tab and page** in `11_testing/tests/` (see `11_testing/tests/README.md` for the file-to-tab map).

Run from **repo root** (or use `11_testing/run_tests.ps1` / `11_testing/run_tests.bat` on Windows):

```bash
# All tests (artifacts + each tab + live API if BASE_URL set)
...
# Only artifact checks
pytest 11_testing/tests/test_artifacts.py -v

# Single tab (e.g. Risk Assessment)
...
- [ ] Run tests with `BASE_URL` set; fix any failing endpoint or missing artifact.
- [ ] **Dashboard errors review:** Open each tab in `status/dashboard_errors` (Causal Analysis, Feature Importance, BupaR, DTW, FP-Growth); run checks in Section 6 and fix any regressions.
...
|-----|-------------------------------------|-----------------|---------------------|
| **Causal Analysis** | `causal_analysis_tab.png` | `test_causal_analysis_tab.py`: GET /visualizations/causal returns 200 and dict; 400 when cohort/age_band missing. | Load tab with cohort/age_band; confirm either causal viz or clear status message (no uncaught error or blank panel). |
| **Feature Importance** | `feature_importance_tab.png` | `test_feature_importance_tab.py`: GET /visualizations/feature_importance returns 200 and heatmap_url/combined_url. Optional: HEAD request to returned URL returns 200. | Load tab; if image fails, status must show "Cohort heatmap image not found. Upload aggregated_fi_heatmap.png to the dashboard bucket." (frontend `onerror`). |
| **BupaR Process Mining** | `PGx Risk Assessment Dashboard_bupaR_tab.pdf` | `test_bupar_tab.py`: GET /visualizations/bupar and GET /visualizations/bupar/activity_frequency return 200 and expected shape; activity_frequency may have null overall/pre_target/post_target. | Load tab; confirm images or friendly message; activity frequency charts show data or "not found" message. |
| **DTW Trajectories** | `PGx Risk Assessment Dashboard_dtw_tab.pdf` | `test_dtw_tab.py`: GET /visualizations/dtw returns 200 and overview_image or chart_data_url or metrics. | Load tab; optional chart_data.json fetch failure must not break tab; status shows error message on API failure. |
| **FP-Growth Patterns** | `PGx Risk Assessment Dashboard_fpgrowth_tab.pdf` | `test_fpgrowth_tab.py`: GET /visualizations/fpgrowth returns 200 with URLs or empty_state. | Load tab; if itemsets image 404s, panel must show "Plot not found. Run 4_dashboard_visuals.ipynb to build visuals." (frontend `onerror`). |
| **PGx Cohort** | (none yet) | `test_cohort_pgx_tab.py`: GET /visualizations/cohort_pgx returns 200 and network_topology_url; 400 when params missing. | Load tab; iframe loads network HTML or status shows "No network URL returned" / error. |
...
2. **Run combination/consensus** if your pipeline has a script that merges SHAP and FFA for the dashboard (e.g. a `combine_shap_ffa_results` or equivalent). Ensure it is run for the cohort/age band in question.
3. **Check CloudWatch logs** for the Lambda function for tracebacks (e.g. `FileNotFoundError`, `MemoryError`) if files exist but 500 still occurs.
...
**Fixes implemented:**
- Frontend sets status `className` to `success` / `error` / `info` and assigns `onerror` on both heatmap and combined images so that a 404 shows the message above instead of a broken image with no feedback.
- Lambda returns 200 with `heatmap_url` and `combined_url`; it does not probe S3, so missing objects result in 404 when the browser loads the image; `onerror` then displays the message.

**Operational checks (to populate heatmaps):**
1. **Generate aggregated heatmaps.** The repo uses `py_helpers/feature_importance_heatmap.py` (e.g. `create_aggregated_fi_heatmap`) which writes `{cohort}_aggregated_fi_heatmap.png` under `outputs_base/{cohort}/plots/`. The deploy notebook (5_build_and_deploy) uploads from there to `feature_importance/{cohort}/aggregated_fi_heatmap.png`. Ensure the feature importance step has been run and the deploy step uploads these files.
2. **If using the R cross-age-band script** (`r_helpers/create_cross_ageband_heatmap.R`): it outputs `{cohort}_{year}_ageband_heatmap_top50.png`. The dashboard expects `aggregated_fi_heatmap.png` per cohort. Either:
   - **Option A:** Copy or rename the R output to `aggregated_fi_heatmap.png` and upload to `feature_importance/{cohort}/aggregated_fi_heatmap.png`, or
   - **Option B:** Use the Python pipeline (`py_helpers/feature_importance_heatmap.py`) which already produces the expected filename.
3. **Combined heatmap:** Deploy notebook uploads `combined_cohorts_feature_importance_heatmap.png` from the local path produced by the same Python helper (e.g. `plots/combined_cohorts_feature_importance_heatmap.png`).

### 7.3 General dashboard deployment verification

After applying fixes or re-running pipelines:
1. **Run the visualization pipeline** so dashboard assets exist: `4_dashboard_visuals.ipynb` or `python 9_dashboard_visuals/run_dashboard_visuals.py` (BupaR, DTW, FP-Growth upload to the dashboard bucket from within the pipeline).
2. **Run the deploy step** so frontend and feature importance heatmaps (and cohort_pgx if used) are synced: `5_build_and_deploy.ipynb` (frontend sync, feature importance upload, cohort_pgx sync).
