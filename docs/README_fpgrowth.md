...
- **Format**: Hive-partitioned Parquet files (event-level cohorts)
- **Size**: ~7 GB, 947 million events

### Source (Model Data for FP-Growth / BupaR / DTW)

After feature importance is computed, we create a compact, analysis-ready dataset in `model_data/`
for each `(cohort_name, age_band)` pair:

- **Target cohort (`opioid_ed`)**:
  - Filter event-level GOLD cohort data to rows where at least one **important item** appears in:
    - `drug_name`
    - `primary_icd_diagnosis_code` through `ten_icd_diagnosis_code`
    - `procedure_code`
  - Write filtered events to:
    - `model_data/cohort_name=opioid_ed/age_band={band}/model_events.parquet`

- **Control cohort (`non_opioid_ed`)**:
  - Load full event-level cohorts for matching age bands and years.
  - Randomly sample patients to maintain an approximate **5:1 control:target ratio**.
...
The FP-Growth cohort pipeline will **prefer `model_data/.../model_events.parquet` when it exists**, and
fall back to the raw GOLD `cohort.parquet` files otherwise.
...
├── cohort_name=opioid_ed/
│   └── age_band=0-12/
│       └── model_events.parquet   # filtered to important items
└── cohort_name=non_opioid_ed/
    └── age_band=0-12/
        └── model_events.parquet   # 5:1 downsampled controls (full feature space)
```
...
│       └── age_band=25-44/
│           └── event_year=2017/
│               ├── itemsets.json                 # Target + background (cases + controls)
│               ├── rules_TARGET_ICD.json         # ← Rules predicting opioid dependence (full background)
│               ├── rules_TARGET_ED.json          # ← Rules predicting ED visits (full background)
│               ├── rules_CONTROL.json            # ← Non-target patterns (baseline care)
│               ├── itemsets_target_only.json     # ← Target-only itemsets (within-case patterns)
│               ├── rules_target_only.json        # ← Target-only rules (no control background)
...
**`itemsets_target_only.json`** / **`rules_target_only.json`** - Target-only patterns:

These files are generated by running FP-Growth **only on target cases** (e.g., `target = 1` in
`model_data/cohort_name=opioid_ed/age_band=.../model_events.parquet`), without any control background.
They answer: *“What patterns are common within target trajectories?”*, while the main
`itemsets.json` / `rules_TARGET_*.json` files answer: *“Which patterns are enriched vs. control?”*
...
- **Source**: All 10 ICD diagnosis columns (primary through ten)
...
### 4. medical_code (Combined ICD + CPT Codes)
- **Source**: Union of all 10 ICD diagnosis columns and `procedure_code`
- **Example items**: "F11.20", "E11.9", "99213", "80053"
- **Purpose**: Capture richer clinical patterns that jointly involve diagnoses and procedures in a single transaction space
