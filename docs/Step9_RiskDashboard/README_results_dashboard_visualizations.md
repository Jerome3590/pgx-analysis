...
- **Feature alignment:** Training features are **n_events**, **item_*** flags (from aggregated feature importance), and **PGx counts** (e.g. pgx_num_drugs, pgx_num_cpic_drugs)—not trajectory, sequence, or itemset columns. Leakage scripts remove those names **defensively** if present. BupaR, FP-Growth, and DTW outputs are for **dashboard visualizations** only. At inference the client sends **cohort, age_band, and selected drugs/ICDs/CPTs** (and optionally age); **n_events**, **pgx_num_drugs**, and **pgx_num_cpic_drugs** are filled from **feature_schema defaults** (e.g. training medians) when omitted so scores stay consistent.
