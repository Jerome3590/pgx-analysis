...
**Project approach:** FP-Growth, BupaR, and DTW are used for **dashboard visualizations only** (Step 9, `10_risk_dashboard/visualizations/`), not as model features. Model features come from aggregated feature importance (Steps 3a–3c), refined in Feature Importance EDA with Step 3c as final update, then model data (Step 4). SHAP (Step 7) and FFA (Step 8) provide post-model interpretability; FFA uses SHAP to prioritize rules.
...
- Features are sorted by `importance_scaled` (recall- or logloss-weighted importance)
...
- Features appearing consistently across **CatBoost, XGBoost, and XGBoost RF** typically have higher importance
...
**Storage Location:**
- `6_bupaR_analysis/sankey_plot.html`
...
- **Dashboard**: The DTW Trajectories tab has an **Event density** dropdown (All | Low | Medium | High | Extreme). After loading visualizations, changing the filter re-renders Routine vs Utilization, Routine event counts, and High-Risk vs Low-Risk Trajectories using the selected density slice when available; "All" uses the aggregate series.
...
- [`README_analysis_workflow.md`](README_analysis_workflow.md) - Feature importance, feature engineering (Steps 5a–5d), and final model workflow
- [`CrossStep_Visualization/README_data_visualization.md`](CrossStep_Visualization/README_data_visualization.md) - Detailed visualization guide
