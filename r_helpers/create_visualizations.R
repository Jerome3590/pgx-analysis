...
create_feature_importance_plots <- function(aggregated_file, 
  ...
  # ---------------------------------------------------------------------------
  # Enrich with feature lookup (if available) for better labels
  # ---------------------------------------------------------------------------
  age_band_fname <- gsub("-", "_", age_band)
  lookup_path <- file.path(here::here(), "feature_encoding_outputs",
                           cohort_name, age_band_fname,
                           sprintf("%s_%s_feature_lookup.csv", cohort_name, age_band_fname))
  
  if (file.exists(lookup_path)) {
    cat(sprintf("Found feature lookup at: %s\n", lookup_path))
    lookup_df <- readr::read_csv(lookup_path, show_col_types = FALSE)
    features_df <- features_df %>%
      dplyr::left_join(lookup_df,
                       by = c("feature" = "feature_name")) %>%
      dplyr::mutate(
        feature_label = dplyr::case_when(
          !is.na(itemset_items) & itemset_items != "" ~ itemset_items,
          TRUE ~ feature
        )
      )
  } else {
    features_df <- features_df %>% dplyr::mutate(feature_label = feature)
  }
  ...
  p1 <- ggplot(top50, aes(x = feature_label, y = importance_scaled)) +
  ...
  p2 <- ggplot(top50_recall, aes(x = feature_label, y = importance_scaled)) +
  ...
  p3 <- ggplot(top50_comparison, aes(x = feature_label, y = value, fill = importance_type)) +
