...
create_all_sequence_features <- function(project_root, cohort_name, age_band, train_label = "train") {
  ...
  age_band_fname <- gsub("-", "_", age_band)
  bupar_output_dir <- file.path(project_root, "5a_bupaR_analysis", "outputs", cohort_name, age_band_fname, "features")
...
# Set output path
feature_eng_dir <- file.path(project_root, "5a_bupaR_analysis", "outputs", "feature_engineering")
