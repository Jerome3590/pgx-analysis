...
- `create_bupar_outputs_opioid_ed.R` / `create_bupar_outputs_non_opioid_ed.R`  
  ...
  - Save per-cohort outputs under `5a_bupaR_analysis/outputs/{cohort}/{age_band_fname}/...` and upload CSVs to S3 `gold/bupar/...`.

- `create_sequence_features.R`  
  - Consumes BupaR trace outputs and builds sequence-based patient features.  
  - Reads from `5a_bupaR_analysis/outputs/{cohort}/{age_band_fname}/features/*traces*_bupar.csv`.  
  - Produces:
    - `sequence_features_{cohort}_{age_band}.csv` under `5a_bupaR_analysis/outputs/feature_engineering/`.  
  - Features include:
...
- `add_bupar_features_to_model_data.R`  
  ...
  - Outputs:
    - `5a_bupaR_analysis/outputs/feature_engineering/bupaR_added_features_{cohort}_{age_band}.csv`, ready to join to model data via `mi_person_key`.
...
- **Inputs** (local):
  ...
  - BupaR trace and feature CSVs under `5a_bupaR_analysis/outputs/{cohort}/{age_band_fname}/features/`.

- **Outputs** (local + S3):
  - Eventlogs, trace tables, process matrices, and plots under `5a_bupaR_analysis/outputs/{cohort}/{age_band_fname}/`.  
  - Final merged feature file:
    - `5a_bupaR_analysis/outputs/feature_engineering/bupaR_added_features_{cohort}_{age_band}.csv`  
