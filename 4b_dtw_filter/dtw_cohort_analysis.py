...
from py_helpers.duckdb_utils import (  # type: ignore[attr-defined]
...
class DTWCohortAnalyzer:
    ...
    def __init__(self, age_band: str, event_year: int, cohort_name: str):
        ...
        self.patient_sequences: Dict[str, List[str]] = {}
        self.cluster_results: Dict[str, object] = {}
        self.analysis_metrics: Dict[str, object] = {}
    ...
    def load_cohort_data(self) -> pd.DataFrame:
        ...
        self.logger.info(
            "Loaded {0} drug exposure records for {1} patients".format(
                len(df), df["mi_person_key"].nunique()
            )
        )
    ...
    def create_drug_sequences(self, df: pd.DataFrame) -> Dict[str, List[str]]:
        ...
        self.logger.info("Created sequences for {0} patients".format(len(sequences)))
    ...
    def encode_drug_sequence(self, drug_sequence: List[str]) -> List[int]:
        ...
        if not hasattr(self, "drug_encoding_map"):
            ...
            self.drug_encoding_map = {
                drug: idx for idx, drug in enumerate(sorted(all_drugs))
            }
            self.logger.info(
                "Created encoding map for {0} unique drugs".format(
                    len(self.drug_encoding_map)
                )
            )

        encoded_sequence = [self.drug_encoding_map.get(drug, -1) for drug in drug_sequence]
        return [x for x in encoded_sequence if x != -1]
    ...
    def calculate_dtw_similarity_matrix(self, sequences: Dict[str, List[str]]) -> np.ndarray:
        ...
        for i in range(n_patients):
            ...
            if (i + 1) % 10 == 0:
                self.logger.info("Processed {0}/{1} patients".format(i + 1, n_patients))
    ...
    def cluster_patients(
        self, similarity_matrix: np.ndarray, n_clusters: int = 5
    ) -> Dict[str, object]:
        """
        Cluster patients based on DTW similarity matrix.
        """
        self.logger.info("Clustering patients into {0} groups...".format(n_clusters))

        finite_matrix = np.where(
            np.isinf(similarity_matrix),
    ...
    def analyze_cluster_characteristics(
        self, df: pd.DataFrame, cluster_results: Dict[str, object]
    ) -> Dict[int, object]:
        """
        ...
        patient_cluster_map = dict(
            zip(
                cluster_results["patient_ids"],  # type: ignore[index]
                cluster_results["cluster_labels"],  # type: ignore[index]
            )
        )

        df_with_clusters = df.copy()
        df_with_clusters["cluster"] = df_with_clusters["mi_person_key"].map(
            patient_cluster_map
        )

        cluster_characteristics: Dict[int, object] = {}

        for cluster_id in range(cluster_results["n_clusters"]):  # type: ignore[index]
            ...
            characteristics = {
                ...
                "avg_sequence_length": cluster_patients.groupby("mi_person_key")
                .size()
                .mean(),
                "most_common_drugs": cluster_patients["drug_name"]
                .value_counts()
                .head(5)
                .to_dict(),
                "therapeutic_class_distribution": cluster_patients["therapeutic_class_1"]
                .value_counts()
                .head(5)
                .to_dict(),
                "avg_days_to_event": cluster_patients["days_to_ade"].mean()
                if "days_to_ade" in cluster_patients.columns
                else None,
                "avg_days_to_opioid_ed": cluster_patients["days_to_opioid_ed"].mean()
                if "days_to_opioid_ed" in cluster_patients.columns
                else None,
            }
    ...
    def create_visualizations(
        self, cluster_results: Dict[str, object], cluster_characteristics: Dict[int, object]
    ) -> None:
        """
        ...
        fig.suptitle(
            "DTW Analysis Results - {0} Cohort ({1}, {2})".format(
        ...
        ax1 = axes[0, 0]
        similarity_matrix = cluster_results["similarity_matrix"]  # type: ignore[index]
        finite_matrix = np.where(
            np.isinf(similarity_matrix),
            np.max(similarity_matrix[~np.isinf(similarity_matrix)]) * 2,
            similarity_matrix,
        ...
        ax2 = axes[0, 1]
        cluster_sizes = cluster_results["cluster_sizes"]  # type: ignore[index]
        ...
        avg_lengths = [
            cluster_characteristics[cid]["avg_sequence_length"] for cid in cluster_ids
        ]
        ...
        if drug_counts:
            top_drugs = sorted(
                drug_counts.items(), key=lambda x: x[1], reverse=True
            )[:10]
        ...
        output_path = (
            "dtw_analysis_results_{0}_{1}_{2}.png".format(
                self.cohort_name, self.age_band, self.event_year
            )
        )
        plt.savefig(output_path, dpi=300, bbox_inches="tight")
        self.logger.info("Visualization saved to {0}".format(output_path))
    ...
    def save_results(
        ...
        results = {
            "metadata": {
                ...
                "n_unique_drugs": len(self.drug_encoding_map)
                if hasattr(self, "drug_encoding_map")
                else 0,
            },
            "clustering_results": {
                "n_clusters": cluster_results["n_clusters"],
                "silhouette_score": cluster_results["silhouette_score"],
                "cluster_sizes": cluster_results["cluster_sizes"].tolist(),  # type: ignore[index]
                "patient_cluster_mapping": dict(
                    zip(
                        cluster_results["patient_ids"],  # type: ignore[index]
                        cluster_results["cluster_labels"],  # type: ignore[index]
                    )
            ...
            "drug_encoding_map": self.drug_encoding_map
            if hasattr(self, "drug_encoding_map")
            else {},
        }

        json_path = (
            f"s3://{S3_BUCKET}/dtw_analysis/"
            f"{self.cohort_name}/{self.age_band}/{self.event_year}/dtw_results.json"
        )
        ...
        parquet_path = (
            f"s3://{S3_BUCKET}/dtw_analysis/"
            f"{self.cohort_name}/{self.age_band}/{self.event_year}/patient_sequences.parquet"
        )
        save_to_s3_parquet(sequences_df, parquet_path)

        self.logger.info("Results saved to S3:")
        self.logger.info(f"  - JSON: {json_path}")
        self.logger.info(f"  - Parquet: {parquet_path}")

    def run_analysis(self, n_clusters: int = 5, create_plots: bool = True) -> Dict[str, object]:
        """
        Run complete DTW analysis pipeline.
        """
        self.logger.info(
            "Starting DTW analysis for {0} cohort...".format(self.cohort_name)
        )
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="DTW Analysis for Drug Sequence Similarity"
    )
    parser.add_argument(
        "--cohort",
        required=True,
    ...
    try:
        ...
        print(f"Number of Patients: {len(results.get('patient_sequences', {}))}")
