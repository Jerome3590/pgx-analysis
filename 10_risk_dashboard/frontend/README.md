...
| **DTW Trajectories** | `chart_data.json`, `sequence_heatmap.json`, `plots/trajectory_overview_plot.json`, `plots/dtw_trajectory_analysis_{base}.png`, `plots/dtw_sample_trajectories_{base}.png`, `plots/dtw_trajectory_cluster_1d/3d_{base}.html` | **JSON-first:** all three JSONs fetched from static (manifest) in parallel; overview/sample use trajectory_overview_plot JSON (Plotly) when present, then PNG from manifest (urls[3],[4]), then interactive HTML (urls[5],[6]). API fallback only when static did not provide data. |
