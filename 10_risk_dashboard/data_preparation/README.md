...
**Outputs:**
- `10_risk_dashboard/visualizations/scenario/{cohort}/{ab}/dashboard_data.json` — merged SHAP + FFA importance, patient-level explanations, consensus features
- S3 (with `--upload-to-dashboard`): `visualizations/scenario/{cohort}/{age_band}/scenario_data.json`

Served by Lambda `GET /visualizations/scenario` endpoint. Also read by `run_dashboard_visuals.py` to build the `allowed_codes_shap_ffa_{cohort}_{ab}.json` that gates BupaR, DTW, and FP-Growth event filtering.
...
**Dashboard S3 (optional):** Use `--upload-to-dashboard` to upload `dashboard_data.json` as `scenario_data.json` to the dashboard bucket (`S3_DASHBOARD_BUCKET` / `S3_DASHBOARD_PREFIX`). The Scenario Analysis tab then loads it via `GET /visualizations/scenario` (prebuilt JSON pattern, same as DTW).
