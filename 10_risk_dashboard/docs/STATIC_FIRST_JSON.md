...
1. **Frontend** requests a **same-origin path** first (e.g. `metadata/opioid_ed.json`, `feature_importance/{cohort}/aggregated_fi_heatmap.json`).
...
| `feature_importance/{cohort}/aggregated_fi_heatmap.json` | Same path (per-cohort or `feature_importance/combined/aggregated_fi_heatmap.json`) | Raw heatmap JSON (same as Lambda response `heatmap_data`). Frontend wraps in `{ heatmap_data, heatmap_url }` for consistent handling. |
...
3. Feature importance: Step 6 uploads `aggregated_fi_heatmap.json` (and PNG) to `feature_importance/{cohort}/` and `feature_importance/combined/` under the dashboard prefix. No wrapper needed; frontend loads that JSON from S3 and uses it as heatmap_data.
