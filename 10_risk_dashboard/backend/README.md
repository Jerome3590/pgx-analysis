...
- `S3_DASHBOARD_PREFIX` - Key prefix for the dashboard app in that bucket (default: `pgx`). BupaR and other visualization APIs use `{prefix}/visualizations/...` (the key must include `visualizations`). **Target path for BupaR plots:** `{prefix}/visualizations/bupar/{cohort}/{age_band}/plots/` — e.g. `s3://jerome-dixon.io/pgx/visualizations/bupar/opioid_ed/45-54/plots/`.
