...
def main():
    parser = argparse.ArgumentParser(
        description="Run opioid_ed cohort creation for all partitions (heavy first)"
    )
    parser.add_argument(
        "--skip-existing",
        action="store_true",
    ...
    if args.skip_existing:
        ...
        # Filter to only opioid_ed (check_existing_cohorts returns jobs for both cohorts)
        # We'll process all jobs since 0_create_cohort.py with --cohort opioid_ed will handle it
    ...
    print(f"\n🚀 Starting opioid_ed cohort creation (heavy partitions first)")
    ...
    for i, job in enumerate(jobs_to_process, 1):
        ...
        print(f"\n{'='*80}")
        print(f"[{i}/{len(jobs_to_process)}] Processing opioid_ed: {job_id}")
        print(f"{'='*80}")
