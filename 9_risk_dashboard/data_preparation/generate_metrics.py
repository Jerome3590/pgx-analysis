...
Reads model_metrics_summary.csv from 6_final_model/outputs (or S3) per cohort/age_band,
aggregates into a single JSON, and writes to 9_risk_dashboard/outputs/metadata/
so it is bundled into the Lambda/ECR image at /var/task/metadata/model_performance_metrics.json.
...
def generate_metrics(download_s3: bool = False) -> Dict[str, Any]:
    ...
    return {"by_cohort": by_cohort, "source": "bundle"}


def main():
    import argparse
    parser = argparse.ArgumentParser(description="Generate model_performance_metrics.json for Lambda bundle.")
