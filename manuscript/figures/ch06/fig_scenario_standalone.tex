...
\definecolor{outgreen}{RGB}{209,250,229}     % outcome box
\definecolor{arrowgray}{RGB}{107,114,128}

\tikzset{
  phasebox/.style={
    rectangle, rounded corners=4pt,
    minimum width=4.1cm, minimum height=8.6cm,
    text width=3.85cm, align=left,
  ...
  arr/.style={-Stealth, thick, arrowgray},
  bigarr/.style={-Stealth, very thick, arrowgray!80},
...
\node[phasebox, fill=obsblue, draw=blue!20,
      below=1.0cm of patient, xshift=-6.35cm] (obs) {%
...
\node[phasebox, fill=origreen, draw=green!30,
      below=1.0cm of patient, xshift=-2.1cm] (ori) {%
...
\node[phasebox, fill=decyellow, draw=yellow!50!gray,
      below=1.0cm of patient, xshift=2.15cm] (dec) {%
...
\node[phasebox, fill=actpink, draw=pink!60!gray,
      below=1.0cm of patient, xshift=6.4cm] (act) {%
...
\draw[arr] (obs.east) -- (ori.west);
\draw[arr] (ori.east) -- (dec.west);
\draw[arr] (dec.east) -- (act.west);

%% ── Outcome box (bottom) ─────────────────────────────────────────────────
\node[botbox, fill=outgreen, draw=green!50!gray,
      below=0.9cm of obs.south |- act.south] (outcome) {%
...
%% ── Arrow from ACT down to outcome ──────────────────────────────────────
\coordinate (midphase) at ($(obs.south)!0.5!(act.south)$);
\draw[bigarr] (midphase) -- (outcome.north);
