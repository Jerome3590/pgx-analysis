...
Outputs:
  data/ontology/articles_screened.csv  — original columns + scoring columns
  scripts/screening_summary.csv        — aggregate counts by RQ

Run from: manuscript/CH_1/Literature_Review/
  python scripts/screen_articles.py [--threshold 0.15] [--use-comprehend]
"""
...
def main():
    ...
    if not tagged_csv.exists():
        sys.exit("ERROR: data/ontology/articles_tagged.csv not found. "
                 "Run organize_by_ontology.R first.")
    ...
    out_path = Path("data/ontology/articles_screened.csv")
    ...
    summary_path = Path("scripts/screening_summary.csv")
    ...
    print(f"\nNEXT: Open data/ontology/articles_screened.csv,")
    print(f"      fill in 'human_decision' column (include/exclude/maybe),")
    print(f"      then re-run prisma_tracker.R with Selected column.")
