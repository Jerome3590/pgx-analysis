...
Usage:
    python 5c_pgx_analysis/run_analysis.py --cohort-name opioid_ed --age-band 0-12
"""
...
def map_drugs_to_genes(
    ...
    with step_block("7_pgx", "map_drugs_to_genes", logger=logger):
        script_path = PROJECT_ROOT / "5c_pgx_analysis" / "map_drugs_to_genes.py"
        ...
        if mapping_file.exists():
            logger.info(
                "Cohort-level drug-gene mapping already exists at %s; skipping for all age bands",
                mapping_file,
            )
            return
        ...
        try:
            ...
            try:
                global_dir = PROJECT_ROOT / "5c_pgx_analysis" / "outputs" / "global"
    ...
    with function_block("7_pgx", "run_pgx_analysis", logger=logger):
        ...
        if not skip_drug_mapping:
            try:
                ...
                # Allele frequencies are now expected to come from the global
                # cache (5c_pgx_analysis/outputs/global/pgx_allele_frequencies_global.csv)
