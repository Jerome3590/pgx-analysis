...
class CatBoostSymbolicExplainer:
    ...
    def fit_from_model_json(self, model_json):
        """Parse CatBoost model and build symbolic CNF clauses for FFA."""
        ...
        # Check for tree types and process accordingly
        if "non_oblivious_trees" in model_json:
            self.logger.info("Processing non-oblivious trees")
            for tree in model_json["non_oblivious_trees"]:
                self._traverse_non_oblivious_tree(tree["nodes"], path=[], conditions=[])
        elif "oblivious_trees" in model_json:
            self.logger.info("Processing oblivious trees")
            for tree in model_json["oblivious_trees"]:
                self._traverse_oblivious_tree(tree)
        else:
            # Try to find trees in a different location in the JSON
            if "trees" in model_json:
                self.logger.info("Processing trees from 'trees' field")
                for tree in model_json["trees"]:
                    if "leaf_values" in tree:
                        self._traverse_oblivious_tree(tree)
                    else:
                        self._traverse_non_oblivious_tree(tree, path=[], conditions=[])
            else:
                raise ValueError("❌ Could not find trees in model JSON. "
                               "Please ensure the model contains either 'oblivious_trees', "
                               "'non_oblivious_trees', or 'trees' field.")

    def _traverse_oblivious_tree(self, tree):
        """Process an oblivious tree and extract CNF rules."""
        def traverse(node, depth=0, conditions=None):
            if conditions is None:
                conditions = []
            
            if "leaf_values" in node:
                # Leaf node - create rules for each leaf value
                for i, leaf_value in enumerate(node["leaf_values"]):
                    pred = 1 if leaf_value > 0 else 0
                    # For oblivious trees, we need to reconstruct the path to this leaf
                    path_conditions = []
                    for j, split in enumerate(tree["splits"][:depth]):
                        if split["split_type"] == "FloatFeature":
                            feature_index = split["float_feature_index"]
                            threshold = split["border"]
                            # Determine if this leaf is in left or right subtree
                            is_left = (i >> (depth - j - 1)) & 1 == 0
                            path_conditions.append((feature_index, threshold, 0 if is_left else 1))
                        elif split["split_type"] == "OneHotFeature":
                            feature_index = split["cat_feature_index"]
                            value = split["value"]
                            is_left = (i >> (depth - j - 1)) & 1 == 0
                            path_conditions.append((feature_index, value, 0 if is_left else 1))
                        elif split["split_type"] == "OnlineCtr":
                            split_index = split["split_index"]
                            threshold = split["border"]
                            is_left = (i >> (depth - j - 1)) & 1 == 0
                            path_conditions.append((split_index, threshold, 0 if is_left else 1))
                    
                    clause = [self._get_condition_literal(f, t, d) for (f, t, d) in path_conditions]
                    self.rule_clauses.append(clause)
                    self.rule_predictions.append(pred)
                return
            
            # Process splits
            if "splits" in tree and depth < len(tree["splits"]):
                split = tree["splits"][depth]
                if split["split_type"] == "FloatFeature":
                    feature_index = split["float_feature_index"]
                    threshold = split["border"]
                    # Left child: feature <= threshold
                    traverse(node["left"], depth + 1, conditions + [(feature_index, threshold, 0)])
                    # Right child: feature > threshold
                    traverse(node["right"], depth + 1, conditions + [(feature_index, threshold, 1)])
                elif split["split_type"] == "OneHotFeature":
                    feature_index = split["cat_feature_index"]
                    value = split["value"]
                    # Left child: feature == value
                    traverse(node["left"], depth + 1, conditions + [(feature_index, value, 0)])
                    # Right child: feature != value
                    traverse(node["right"], depth + 1, conditions + [(feature_index, value, 1)])
                elif split["split_type"] == "OnlineCtr":
                    split_index = split["split_index"]
                    threshold = split["border"]
                    # Left child: ctr <= threshold
                    traverse(node["left"], depth + 1, conditions + [(split_index, threshold, 0)])
                    # Right child: ctr > threshold
                    traverse(node["right"], depth + 1, conditions + [(split_index, threshold, 1)])
    ...
    def _traverse_non_oblivious_tree(self, node, path, conditions):
        ...
        if split_type == "FloatFeature":
            ...
            # Left child: condition is "feature == value" => direction=0
            self._traverse_non_oblivious_tree(
                node["left"],
                path + [0],
                conditions + [(feature_index, value, 0)]
            )
            
            # Right child: condition is "feature != value" => direction=1
            self._traverse_non_oblivious_tree(
                node["right"],
                path + [1],
                conditions + [(feature_index, value, 1)]
            )
            ...
            # Left child: condition is "ctr <= threshold" => direction=0
            self._traverse_non_oblivious_tree(
                node["left"],
                path + [0],
                conditions + [(split_index, threshold, 0)]
            )
            
            # Right child: condition is "ctr > threshold" => direction=1
            self._traverse_non_oblivious_tree(
                node["right"],
                path + [1],
                conditions + [(split_index, threshold, 1)]
            )
