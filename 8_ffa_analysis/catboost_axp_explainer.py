...
class CatBoostSymbolicExplainer(BaseSymbolicExplainer):
    ...
    def fit_from_model_json(self, model_json):
        ...
        # === Handle oblivious trees ===
        if "oblivious_trees" in model_json:
            # CatBoost models with oblivious trees can still be processed
            # Use dataframe approach for easier debugging
            self.logger.info("Model uses oblivious trees - processing with dataframe approach")
            for tree_idx, tree in enumerate(model_json["oblivious_trees"]):
                try:
                    df_paths = self._explode_oblivious_tree_to_dataframe(tree, tree_idx)
                    if len(df_paths) > 0:
                        self._create_rules_from_dataframe(df_paths)
                    else:
                        self.logger.warning(f"Oblivious tree {tree_idx}: DataFrame explosion returned empty")
                except Exception as e:
                    self.logger.error(f"Error processing oblivious tree {tree_idx}: {e}")
                    import traceback
        ...
        if "non_oblivious_trees" not in model_json:
            raise ValueError("[ERROR] Model JSON missing 'non_oblivious_trees'. "
    ...
    def _explode_tree_to_dataframe(self, tree: Dict, tree_idx: int = 0) -> pd.DataFrame:
        """
        Explode a CatBoost tree into unified DataFrame schema.
        Handles both oblivious and non-oblivious trees.
        """
        # Check if this is an oblivious tree (has 'splits' and 'leaf_values')
        if "splits" in tree and "leaf_values" in tree:
            return self._explode_oblivious_tree_to_dataframe(tree, tree_idx)
        else:
            # Non-oblivious tree - convert to standard format first
            # For now, use recursive traversal and convert to DataFrame
            # This is a fallback - ideally non-oblivious trees should also use DataFrame
            self.logger.warning(f"Non-oblivious tree {tree_idx}: Using fallback traversal method")
            return pd.DataFrame()  # Empty - will use traversal method
    
    def _explode_oblivious_tree_to_dataframe(self, tree: Dict, tree_idx: int = 0) -> pd.DataFrame:
        """
        Explode a CatBoost oblivious tree into a DataFrame where each row represents a path from root to leaf.
        Oblivious trees have all nodes at the same depth use the same split condition.
        
        Structure:
        - splits: List of split conditions (one per depth level)
        - leaf_values: List of leaf values (one per leaf, ordered by binary path)
        ...
        if not splits or not leaf_values:
            self.logger.warning(f"Oblivious tree {tree_idx}: Missing splits or leaf_values")
            return pd.DataFrame()
        
        # Each leaf corresponds to a unique path through the tree
        # For oblivious trees, the path is determined by the binary representation of the leaf index
        num_leaves = len(leaf_values)
        depth = len(splits)
        
        for leaf_idx in range(num_leaves):
            # Convert leaf index to binary path (left=0, right=1)
            # For oblivious trees, leaf_idx directly encodes the path
            path_binary = []
            temp_idx = leaf_idx
            for _ in range(depth):
                path_binary.append(temp_idx % 2)
                temp_idx //= 2
            
            # Reverse to get path from root to leaf
            path_binary = path_binary[::-1] if depth > 0 else []
            
            # Build conditions for this path
            conditions_for_path = []
            for depth_level, direction in enumerate(path_binary):
                if depth_level < len(splits):
                    split = splits[depth_level]
                    
                    # Extract feature and threshold from split
                    if "float_feature_index" in split:
                        feature_idx = split["float_feature_index"]
                        threshold = split.get("border", split.get("threshold", 0.0))
                    elif "cat_feature_index" in split:
                        # Categorical split - skip for now or handle differently
                        self.logger.warning(f"Oblivious tree {tree_idx}: Categorical split at depth {depth_level}, skipping")
                        continue
                    else:
                        self.logger.warning(f"Oblivious tree {tree_idx}: Unknown split format at depth {depth_level}")
                        continue
                    
                    conditions_for_path.append({
                        'feature_idx': feature_idx,
                        'feature_name': self.feature_names.get(feature_idx, f"f{feature_idx}"),
                        'threshold': threshold,
                        'direction': direction,  # 0 for left (<=), 1 for right (>)
                        'depth': depth_level
                    })
            
            # Get leaf value and prediction
            leaf_value = leaf_values[leaf_idx]
            pred = 1 if leaf_value > 0 else 0
            
            # Create rows for each step in the path
            for step_idx, step in enumerate(conditions_for_path):
                paths.append({
                    'tree_idx': tree_idx,
                    'path_idx': leaf_idx,  # Unique path identifier (leaf index)
                    'step_in_path': step_idx,
                    'feature_idx': step['feature_idx'],
                    ...
                    'prediction': pred,
                    'path_length': len(conditions_for_path)
                })
            ...
            if conditions_for_path:
                paths.append({
                    'tree_idx': tree_idx,
                    'path_idx': leaf_idx,
                    'step_in_path': len(conditions_for_path),
                    'feature_idx': None,
                    'feature_name': 'LEAF',
                    'threshold': None,
                    'direction': None,
