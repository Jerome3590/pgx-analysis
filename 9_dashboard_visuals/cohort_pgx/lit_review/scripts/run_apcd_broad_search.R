# Broader APCD searches with multiple query variations
library(rentrez)
library(dplyr)
library(readr)
library(xml2)
...
queries <- list(
  "all payers claim database" = "all payers claim database",
  "all payer claims database" = "all payer claims database", 
  "APCD all payer" = "APCD all payer",
  "all payer claims database machine learning" = "all payer claims database machine learning",
  "all payer claims database predictive" = "all payer claims database predictive",
  "claims database healthcare predictive modeling" = "claims database healthcare predictive modeling"
)

all_results <- tibble()

cat("=== Testing Multiple APCD Search Queries ===\n\n")

for (name in names(queries)) {
  query <- queries[[name]]
  filename <- paste0("apcd_", gsub(" ", "_", name), "_articles.csv")
...
if (nrow(all_results) > 0) {
  ...
  write_csv(all_results, "apcd_analysis_articles_combined.csv")
  cat("=== Combined Results ===\n")
  cat("Total unique articles:", nrow(all_results), "\n")
  cat("Saved to: apcd_analysis_articles_combined.csv\n")
} else {
