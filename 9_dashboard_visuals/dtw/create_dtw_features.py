...
# Step folder (9_dashboard_visuals) and repo root; outputs go to 10_risk_dashboard/visualizations/dtw
...
def extract_patient_trajectories(
    ...
    max_lookback_months: Optional[int] = None,
) -> Dict[str, List[str]]:
    """
    Extract patient trajectories from model_data (target-aligned when cutoff_dates set).

    For target-event pathway analysis: anchor = target event date; only events *before*
    that date are included. Optional max_lookback_months (e.g. 24) restricts to the
    N months prior to the anchor so pathways are comparable on-ramps.
    ...
    cutoff_dates : Optional[Dict[str, str]]
        Dictionary mapping mi_person_key to cutoff date (anchor = target event; events before this date)
    max_lookback_months : Optional[int]
        If set (e.g. 24), for target patients only include events in the N months *prior* to cutoff_date.
        None = no lookback limit (all events before anchor). Controls (NULL cutoff) unchanged.
    ...
    if cutoff_dates:
        ...
        if not cutoff_df.empty:
            ...
            """
            if max_lookback_months is not None and max_lookback_months > 0:
                # Target-event pathway: only events in the N months prior to anchor (target patients)
                cutoff_where = f"""
            AND (cd.cutoff_date IS NULL OR (
                e.event_date < CAST(cd.cutoff_date AS TIMESTAMP)
                AND e.event_date >= (CAST(cd.cutoff_date AS DATE) - INTERVAL '{max_lookback_months} months')
            ))
            """
            else:
                cutoff_where = """
...
def create_all_dtw_features(
    ...
    max_lookback_months: Optional[int] = 24,
) -> pd.DataFrame:
    """
    Create all DTW features for specified item types (target-event pathway analysis).

    Uses target-aligned trajectories: anchor = first target event date; optional
    max_lookback_months (default 24) restricts to the N months prior to anchor.
    ...
    max_lookback_months : Optional[int]
        For target patients, only include events in the N months prior to the anchor (default 24).
        None = no lookback limit (all events before anchor).
...
def main():
    ...
    if args.research_mode:
        ...
        logger.info("\n" + "=" * 80)
        logger.info("Research mode complete. Review outputs in 10_risk_dashboard/visualizations/dtw/outputs/research/")
        logger.info("=" * 80)
        return
    
    # Normal mode: Create DTW features (target-event pathway; 24-month lookback default)
    max_lookback = args.max_lookback_months if args.max_lookback_months > 0 else None
    dtw_features = create_all_dtw_features(
        ...
        max_lookback_months=max_lookback,
    )
