#!/usr/bin/env python3
"""
Local Python workflow for dashboard visuals (pipeline step 10).
...
Usage:
  ...
  python 10_dashboard_visuals/run_dashboard_visuals.py --sync-only

  # Skip sync (data already local), run visuals
  python 10_dashboard_visuals/run_dashboard_visuals.py --no-sync

  # One cohort/age_band, force re-run
  python 10_dashboard_visuals/run_dashboard_visuals.py --no-sync --cohort opioid_ed --age-band 13-24 --force

  # AWS profile for sync
  python 10_dashboard_visuals/run_dashboard_visuals.py --profile my-profile
"""
...
def run_sync(profile: str | None) -> bool:
    ...
    script = REPO_ROOT / "10_dashboard_visuals" / "sync_visualization_data_from_s3.py"
...
def main():
    ...
    print("Dashboard visuals workflow (step 10)")
    print("=" * 60)
    ...
    vis_root = REPO_ROOT / "9_risk_dashboard" / "visualizations"
    bupar_script = vis_root / "bupar" / "create_bupar_visuals.py"
    dtw_features_script = vis_root / "dtw" / "create_dtw_features.py"
    dtw_visuals_script = vis_root / "dtw" / "create_dtw_visuals.py"
    fpgrowth_script = vis_root / "fpgrowth" / "create_fpgrowth_visuals.py"
