---
title: "Build PGx Database (DEPRECATED - Use Python Scripts)"
author: "Jerome Dixon"
date: "12/8/2020"
output: html_document
---

**⚠️ DEPRECATED**: This R script uses outdated PharmGKB API endpoints and URLs.

**Use the new Python implementation instead:**
- `fetch_pharmgkb_data.py` - Fetch VIP data from current PharmGKB API
- `generate_pgx_qr_codes.py` - Generate QR codes with ClinPGx URLs
- `build_pgx_database.py` - Build unified PGx database
- See `README_PYTHON.md` for full documentation

**Changes in 2026:**
- PharmGKB API `/v1/site/vips` endpoint no longer available
- VIP pages migrated to ClinPGx: `https://www.clinpgx.org/vip/{PA_ID}/overview`
- CPIC Excel file still valid: `https://files.cpicpgx.org/data/report/current/pair/cpic_gene-drug_pairs.xlsx`
...
# Note: PharmGKB API has migrated. The /v1/site/vips endpoint is deprecated.
# Current API documentation: https://www.postman.com/pharmgkb/pharmgkb-api/documentation/g9rp4zr/pharmgkb-rest-api
# VIP content now surfaces at ClinPGx URLs (clinpgx.org) instead of pharmgkb.org

build_VIP_df <- function() {
  
  # WARNING: This endpoint (/v1/site/vips) is deprecated as of 2026.
  # For current data, use entity-specific endpoints from PharmGKB REST API.
  # This code is retained for legacy compatibility.
  ...
  # Updated: PharmGKB VIP content now hosted on ClinPGx
  base_url <- "https://www.clinpgx.org/vip/"
  
  vip_df$URL <- paste0(base_url, vip_df$Vipid, "/overview", sep="")
